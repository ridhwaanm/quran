...
import com.ridhwaan.quran.model.Bookmark
            ...
            ayahBookmarks.forEach { bookmark ->
                bookmark.ayahRef?.let { ayahRef ->
                    ...
                    if (rect != null) {
                        ...
                        // Use absolute offset instead of the Box wrapping technique
                        Box(
                                modifier =
                                        Modifier.absoluteOffset(x = rect.left.dp, y = rect.top.dp)
                                                .size(40.dp)
                                                .zIndex(
                                                        9999f
                                                ) // Make sure it's on top of everything
                        ) {
                            // Larger red circle for better visibility
                            Surface(
                                    modifier = Modifier.size(30.dp).align(Alignment.Center),
                                    shape = CircleShape,
                                    color = Color.Red // Fully opaque
                            ) {}

                            // Bookmark icon on top of the red circle
                            IconButton(
                                    onClick = {
                        ...
                        android.util.Log.d(
                                tag,
                                "Rendered ayah bookmark for $ayahRef at (${rect.left}, ${rect.top})"
                        )
                    } else {
